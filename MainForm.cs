    ...
    public class MainForm : System.Windows.Forms.Form
    {
        ...
        private void startButton_Click(object sender, System.EventArgs e)
        {
            if (runningTask == null)
            {
                try
                {
                    samplingRate = Convert.ToDouble(rateNumeric.Value);
                    samplingCount = Convert.ToInt32(samplesPerChannelNumeric.Value);
                    timeInterval = 1 / samplingRate;
                    rowIndexes = new int[samplingCount];
                    times = new double[samplingCount];
                    ...
                    myTask.Timing.ConfigureSampleClock("", Convert.ToDouble(rateNumeric.Value), //sampling rate
                    ...
                    analogInReader.BeginReadWaveform(Convert.ToInt32(samplesPerChannelNumeric.Value),  //sampling count
        ...
        private void dataToDataTable(AnalogWaveform<double>[] sourceArray, ref DataTable dataTable)
        {
            ...
            foreach (AnalogWaveform<double> waveform in sourceArray)
            {
                ...
                {
                    data.Add(waveform.Samples[sample].Value);
                ...
                if (data.Count == waveform.Samples.Count)
                {
                    dataSet.Add(data.ToArray());              
                }
                currentLineIndex++;
            }
            //double[][] vs = dataSet.ToArray();

            int a = 0;
            sd.createCSVfile(rowIndexes, times, dataSet.ToArray()); 
        }
